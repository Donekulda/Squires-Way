...
SquiresWay is structured in layers: **core** at the bottom (types), **io** and **storage** for paths and files, **parser** for search/filter logic, **merging** for pure merge helpers, **platform** for OS/UI-shell settings and clipboard contracts (no dependency on other crate modules), and **services** on top for resolution and glue. **`lib.rs`** exposes modules to the binary and tests; **`main.rs`** is the process entry until the Bevy UI is wired.
...
- **platform** → (nothing in-crate): configuration DTOs, Linux display helpers, clipboard traits; consumed by future UI and **`game`**.
...
- **merging** → (nothing in-crate): merge logic for parser pipeline integration.
...
3. **Platform settings**: App or JSON store → `platform::PlatformConfigurationOptions` (serde) → UI / future `game` (Linux display options, clipboard policy, etc.).
